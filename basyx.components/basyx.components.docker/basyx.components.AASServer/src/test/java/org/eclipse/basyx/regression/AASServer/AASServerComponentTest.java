...
public class AASServerComponentTest {
	...
	public void testServerCleanup() {
		...
		List<AASDescriptor> aasDescriptors = registry.lookupAllShells();
		...
		for(AASDescriptor aasDescriptor: aasDescriptors) {
			try {
				registry.lookupShell(aasDescriptor.getIdentifier());
				fail();
			...
			for(SubmodelDescriptor smDescriptor: aasDescriptor.getSubmodelDescriptors()) {
				try {
					registry.lookupSubmodelForShell(aasDescriptor.getIdentifier(), smDescriptor.getIdentifier());
					fail();
				} catch (ResourceNotFoundException e) {
				}
			}
